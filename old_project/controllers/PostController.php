...
class PostController extends BaseController
{
    ...
    public function index(): void
    {
        ...
        if ($q) {
            $rawPosts = $this->postModel->searchFullText($q);
        } else {
            $rawPosts = $this->postModel->all('published');
        }

        // Recorre cada post y le busco la nota media de estrellas que le han dado
        $posts = array_map(function ($post) {
            $post['avg_rating'] = $this->commentModel->averageRating((int) $post['id']);
            return $post;
        }, $rawPosts);

        // Ordena los posts por mejor nota y coge los 3 mejores para ponerlos arriba del todo
        $featured = $posts;
    ...
    public function show(): void
    {
        ...
        if (($post['status'] ?? 'published') === 'draft') {
            if (empty($_SESSION['user_id']) || (!$this->isAdmin() && $post['user_id'] !== $_SESSION['user_id'])) {
                http_response_code(404); // Si es un borrador y no eres tu, no te dejo verlo
                echo 'Post no encontrado';
                return;
            }
        }
        ...
        // BUSCADOR INTELIGENTE: Busco otros libros que se parezcan a este
    ...
    {
        ini_set('memory_limit', '256M'); // Le da mas memoria al servidor para que no pete con fotos grandes
    ...
    public function create(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                // AVISO AUTOMATICO: Le dice a n8n que mande el email a todo el mundo
